...
class UserLogin{
    ...
    public $valor_frete_rodo_sem_taxa;
    ...
    public function check_userlogin() {
        ...
        if (empty($userdata)) {
            ...
            $valor_frete_rodo_sem_taxa = 'teste';
        ...
        if ($post) {
            ...
            $sql = "SELECT 
                    ...
                    , pes.Email
                    , pes.Senha as senha
                    , og.idOperadorGrupoNomenclatura as idGrupo
                    , og.Nome as Grupo
                    FROM operadores o
                    INNER JOIN pessoas pes ON pes.idPessoa = o.idPessoa
                    LEFT JOIN operadores_grupo_nomenclaturas og ON og.idOperadorGrupoNomenclatura = o.idOperadorGrupo AND og.DataDelete IS NULL
                    WHERE o.DataDelete IS NULL
                            AND pes.DataDelete IS NULL
                            AND pes.Email = ?
                            AND pes.Senha = ?;";
            ...
            $dadosUsuario = $query->fetchAll();            
            // $dadosUsuario = $query->fetch(PDO::FETCH_ASSOC);

            $dadosUsuario = (isset($dadosUsuario[0]) ? $dadosUsuario[0] : null);

            // Obtém o ID do usuário
            $idPessoa = (int) $dadosUsuario['idPessoa'];
            ...
            // Obtém um array com as permissões de usuário
                ...
                //                if($_POST['nivel'] == 'A'){
                //                   $remove ='professor';
                //                }elseif($_POST['nivel'] == 'P'){
                //                   $remove ='alunoElos';
                //                }
                //
                //                $input = unserialize( $permissoes );
                //                $key = array_search($remove, $input);
                //                if($key!==false){
                //                    unset($input[$key]);
                //                }
                //                $_SESSION['userdata']['user_permissions'] = $input;
                //            }else{
                //                $_SESSION['userdata']['user_permissions'] = unserialize( $permissoes );
                //            }
